...
@limiter.limit(f"{settings.RATE_LIMIT_PER_MINUTE}/minute")
def get_lead(request: Request, lead_id: int, db: Session = Depends(get_db)):
...
@limiter.limit(f"{settings.RATE_LIMIT_PER_MINUTE}/minute")
def update_lead(request: Request, lead_id: int, lead_update: LeadUpdate, db: Session = Depends(get_db)):
    lead = db.query(HighEquityLead).filter(HighEquityLead.id == lead_id).first()
    if not lead:
        raise HTTPException(status_code=404, detail="Lead not found")
